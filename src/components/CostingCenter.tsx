...
export const RecipeCostTab: React.FC = () => {
  ...
  const [workingDaysDraft, setWorkingDaysDraft] = useState<number>(30);
  const [busy, setBusy] = useState(false);

  // ต้นทุนคงที่รายเดือน (ค่าแรง/ค่าเช่า/ค่าไฟ) — ตามหลักเอกสาร CostRecipe:
  // ไม่บวกเข้าต้นทุนต่อถาด แต่ใช้คิด "จุดคุ้มทุน กี่ถาด/วัน"
  const fixedMonthly = (costSettings.overheadLines || []).reduce((s, l) => s + (Number(l.amount) || 0), 0);
  const workingDays = Math.max(1, Number(costSettings.workingDays) || 30);
  const fixedPerDay = fixedMonthly / workingDays;

  // กำไรขั้นต้นเฉลี่ยต่อถาด จากเมนูที่มีสูตรแล้ว → จุดคุ้มทุน
  const avgGross = useMemo(() => {
    const rows = (menu || [])
      .map(m => ({ price: Number(m.basePrice) || 0, cost: recipeFoodCost(m.id) }))
      .filter(r => r.cost !== null && r.price > 0);
    if (rows.length === 0) return 0;
    return rows.reduce((s, r) => s + (r.price - (r.cost as number)), 0) / rows.length;
  }, [menu, menuRecipes, stockItems]);
  const breakevenTrays = avgGross > 0 ? Math.ceil(fixedPerDay / avgGross) : 0;
  const traysForProfit = (target: number) => avgGross > 0 ? Math.ceil((fixedPerDay + target / workingDays) / avgGross) : 0;
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-5">
      {/* Fixed costs + breakeven card (ตามหลัก CostRecipe.docx) */}
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
        <div className="flex flex-wrap justify-between items-center gap-2">
          <div className="text-sm font-black text-amber-900 flex items-center gap-2">
            <Calculator size={16} />
            {th ? `ต้นทุนคงที่: ${fmtB(fixedMonthly)}/เดือน (≈${fmtB(fixedPerDay)}/วัน)` : `Fixed costs: ${fmtB(fixedMonthly)}/mo (≈${fmtB(fixedPerDay)}/day)`}
          </div>
          <button onClick={() => { setOverheadDraft(JSON.parse(JSON.stringify(costSettings.overheadLines || []))); setWorkingDaysDraft(workingDays); setShowOverhead(true); }}
        ...
        <div className="flex flex-wrap gap-1.5 mt-2">
          ...
          <span className="text-[10px] font-bold bg-white border border-amber-200 text-amber-800 rounded-full px-2 py-0.5">📅 {th ? `เปิด ${workingDays} วัน/เดือน` : `${workingDays} days/mo`}</span>
        </div>
        {avgGross > 0 && (
          <div className="mt-3 bg-white border-2 border-amber-300 rounded-xl p-3 grid grid-cols-2 sm:grid-cols-4 gap-2 text-center">
            <div><div className="text-lg font-black text-amber-900">{breakevenTrays}</div><div className="text-[9px] font-bold text-amber-700">{th ? 'ถาด/วัน = คุ้มทุน' : 'trays/day breakeven'}</div></div>
            <div><div className="text-lg font-black text-amber-900">{traysForProfit(50000)}</div><div className="text-[9px] font-bold text-amber-700">{th ? 'ถาด/วัน = กำไร 50k/ด.' : 'for 50k/mo'}</div></div>
            <div><div className="text-lg font-black text-amber-900">{traysForProfit(100000)}</div><div className="text-[9px] font-bold text-amber-700">{th ? 'ถาด/วัน = กำไร 100k/ด.' : 'for 100k/mo'}</div></div>
            <div><div className="text-lg font-black text-amber-900">{fmtB(avgGross)}</div><div className="text-[9px] font-bold text-amber-700">{th ? 'กำไรขั้นต้นเฉลี่ย/ถาด' : 'avg gross/tray'}</div></div>
          </div>
        )}
        <p className="text-[10px] text-amber-700 mt-2 flex items-start gap-1">
          <Info size={11} className="shrink-0 mt-0.5" />
          {th ? 'ค่าแรง/ค่าเช่า/ค่าไฟ ไม่ถูกบวกในต้นทุนต่อถาด (ตามหลัก Food Cost%) — งบกำไรขาดทุนใช้รายจ่ายจริงจากแท็บรายจ่าย ตัวเลขนี้ไว้ดูจุดคุ้มทุน' : 'Fixed costs are not per-tray — used for breakeven only; P&L uses real expenses.'}
        </p>
      </div>
        ...
        <div key={cat} className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-gray-50">
            {items.map((m: any) => {
              ...
              const foodCostPct = hasRecipe && price > 0 ? ((cost as number) / price) * 100 : null;
              return (
                <button key={m.id} onClick={() => openEditor(m.id)}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[11px] text-gray-400 font-semibold">
                      ...
                      {hasRecipe && <span> • Food Cost <b className={(foodCostPct as number) <= 35 ? 'text-emerald-600' : (foodCostPct as number) <= 40 ? 'text-amber-600' : 'text-red-600'}>{Math.round(foodCostPct as number)}%</b></span>}
                    </div>
                  </div>
                  {hasRecipe ? (
          ...
          <div className="bg-white rounded-2xl w-full max-w-xl max-h-[92vh] flex flex-col shadow-2xl">
            ...
            <div className="px-5 py-3 border-t border-gray-100 space-y-2">
              <div className="flex justify-between text-sm font-black">
                <span className="text-gray-500">{th ? 'ต้นทุนต่อถาด (รวมกล่อง+แก๊ส)' : 'Cost per tray'}</span>
                <span className="text-gray-800">{fmtB(editCost)}</span>
              </div>
              <div className="flex justify-between text-[11px] font-bold text-gray-400">
                <span>Food Cost <b className={editPrice > 0 && (editCost / editPrice) * 100 <= 35 ? 'text-emerald-600' : 'text-red-500'}>{editPrice > 0 ? Math.round((editCost / editPrice) * 100) : 0}%</b> {th ? '(มาตรฐานร้านพิซซ่า 30-35%)' : '(industry 30-35%)'}</span>
      ...
      {showOverhead && (
        <div className="fixed inset-0 bg-black/60 z-[95] flex items-center justify-center p-3">
          <div className="bg-white rounded-2xl w-full max-w-md max-h-[90vh] flex flex-col shadow-2xl">
            <div className="px-5 py-3.5 border-b border-gray-100 flex justify-between items-center">
              <h4 className="font-black text-gray-800 flex items-center gap-2"><Calculator size={16} className="text-amber-600" />{th ? 'ต้นทุนคงที่รายเดือน' : 'Monthly fixed costs'}</h4>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              ...
              <button onClick={() => setOverheadDraft(prev => [...prev, { name: '', amount: 0 }])}
                ...
                <Plus size={14} />{th ? 'เพิ่มรายการ (฿/เดือน)' : 'Add line (per month)'}
              </button>
              <div className="flex items-center justify-between gap-3 pt-1">
                <label className="text-xs font-bold text-gray-500 flex items-center gap-2">
                  📅 {th ? 'เปิดร้านกี่วัน/เดือน' : 'Working days/mo'}
                  <input type="number" min={1} max={31} className={`${inputCls} !w-20 text-center font-black`} value={workingDaysDraft}
                    onChange={e => setWorkingDaysDraft(Number(e.target.value))} />
                </label>
                <div className="text-right text-sm font-black text-gray-700">
                  {fmtB(overheadDraft.reduce((s, l) => s + (Number(l.amount) || 0), 0))}/{th ? 'เดือน' : 'mo'}
                  <div className="text-[10px] text-gray-400">≈ {fmtB(overheadDraft.reduce((s, l) => s + (Number(l.amount) || 0), 0) / Math.max(1, workingDaysDraft))}/{th ? 'วัน' : 'day'}</div>
                </div>
              </div>
            </div>
            <div className="px-5 py-3 border-t border-gray-100 flex gap-2">
              ...
              <button onClick={async () => {
                setBusy(true);
                const ok = await saveCostSettings({ overheadLines: overheadDraft.filter(l => l.name.trim()), workingDays: workingDaysDraft });
                setBusy(false);
